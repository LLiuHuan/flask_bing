...
logging.info('当前时间：%s 程序已经执行' % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),))
def addslashes(s):
    d = {'"': '\\"', "'": "\\'", "\0": "\\\0", "\\": "\\\\"}
...
sql += " enddate,hsh,url,imgUrl,http,bot,drk,title,top,wp,hs,addTime) "
sql += " values ('%s','%s','%s','%s','%s','%s','%s','%s','%s',%d,%d,'%s',%d,'%s','%s',SYSDATE()) " % \
       (str(res_json['copyright']), str(res_json['copyrightlink']), str(res_json['startdate']),
        ...
        addslashes('/data/www/images/'+img), http, int(res_json['bot']), int(res_json['drk']), str(res_json['title']),
